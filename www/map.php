...
switch ($tooltipp) {
  case 'diff':
    $tooltip = "d.name + ': ' + d.winner + ' won by ' + d.population + ' votes'";
    break;
  case 'ring';
    $tooltip = "d.name + ': ' + d.winner + ' won ' + Math.max(d.population.p6,d.population.p9) + ':' + Math.min(d.population.p6,d.population.p9)";
    break;
  default:
    $tooltip = "d.name + ': ' + d.winner + ' (' + d.population + ')'";
}

if ($template) $tpl = 'mapx_' . $template . '.tpl';
else $tpl='mapx.tpl';
...
if (($election == 'president_2013') and ($legend == 'diff')) {
  $description = '<em>Differences between numbers for <span class="zeman">Mr. Zeman</span> and <span class="schwarzenberg">Mr. Schwarzenberg</span></em>
  <p>We can identify regions, where the candidates were gaining votes on each other:
  <h3>Mr.Zeman was gaining on Mr.Schwarzenberg</h3>
  <ul><li>Ostrava region</li>
  <li>vast area including whole Moravia (with exception of main cities) over Vysočina to Kolín in Central Bohemia</li>
  <li>Ústí nad Labem region</li>
  </ul>
  <h3>Mr.Schwarzenberg was gaining on Mr.Zeman</h3>
  <ul><li>Prague and its outskirts</li>
  <li>Brno</li>
  <li>Plzeň, Liberec+Jablonec and Hradec Králové</li>
  </ul>
   </p>';
}
...
$smarty->assign('map_id',$_REQUEST['map_id']);
